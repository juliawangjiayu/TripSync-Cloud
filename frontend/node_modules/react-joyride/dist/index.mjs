...
function omit(input, ...filter) {
 ...
 /* istanbul ignore else */
	if ({}.hasOwnProperty.call(input, key)) {
		if (!filter.includes(key)) output[key] = input[key];
	}
	return output;
}
/**
...
function useLifecycleEffect(options) {
	...
	useEffect(() => {
		...
		if (hasChangedTo("action", [
			ACTIONS.NEXT,
			ACTIONS.PREV,
			ACTIONS.SKIP,
			ACTIONS.CLOSE
		]) || lastAction.current === ACTIONS.CLOSE && action === ACTIONS.START) lastAction.current = action;
	}, [action]);
	useEffect(() => {
		if (!previousStateRef.current) return () => {
			cleanup();
...
function TourRenderer({ controls, mergedProps, state, step, store }) {
	...
	const handleClickOverlay = useCallback(() => {
		if (step?.overlayClickAction === "close") controls.close(ORIGIN.OVERLAY);
		else if (step?.overlayClickAction === "next") controls.next(ORIGIN.OVERLAY);
